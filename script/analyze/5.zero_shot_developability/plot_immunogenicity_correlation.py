#!/usr/bin/env python3
"""
Immunogenicity Correlation Analysis with Bootstrapping

This script creates publication-quality bar plots comparing Spearman and Pearson
correlations between protein language model PPL scores and immunogenicity metrics.

Key Features:
1. Bootstrapping (1000 iterations) for 95% confidence intervals
2. Paired bootstrap test for statistical significance vs PRISM
3. Raw PPL values used (no direction correction)
4. Generates single figure with Spearman and Pearson bar plots

Correlation Direction:
    - PPL: Lower = better (more natural/human-like sequence)
    - Immunogenicity: Lower = better (less immune response)
    - Expect negative correlations: Higher PPL → Higher immunogenicity

Usage:
    python plot_immunogenicity_correlation.py \\
        --csv_path data/ginkgo/immunogenicity.csv

    python plot_immunogenicity_correlation.py \\
        --csv_path data/ginkgo/immunogenicity.csv \\
        --target_column immunogenicity_score \\
        --n_bootstrap 1000
...
# Unified font configuration for publication-quality figures
FONT_CONFIG = {
    # Axis labels
    'label_size': 25,
    'label_weight': 'bold',
...
def bootstrap_correlation(
    ...
    Args:
        ...
        y: Second variable array (immunogenicity)
...
def paired_bootstrap_test(
    ...
    Args:
        ...
        y: Target variable (immunogenicity)
        method: 'spearman' or 'pearson'
        n_bootstrap: Number of bootstrap iterations
...
def create_correlation_barplot(
    ...
    ax.set_ylabel(ylabel, fontsize=FONT_CONFIG['label_size'],
                  fontweight=FONT_CONFIG['label_weight'])

    if title:
        ax.set_title(title, fontsize=FONT_CONFIG['title_size'],
                     fontweight=FONT_CONFIG['title_weight'], pad=10)
    ...
    for i, (bar, corr, ci_high, ci_low) in enumerate(zip(bars, corr_values, ci_highs, ci_lows)):
        ...
        ax.text(bar.get_x() + bar.get_width()/2., y_pos,
                ...
                fontweight=FONT_CONFIG['annotation_weight'], color='black')

    # Set y-axis limits
    y_upper = max(0.5, max_bracket_y + 0.15)
    ...
    for label in ax.get_yticklabels():
        label.set_fontweight(FONT_CONFIG['tick_weight'])
...
def analyze_immunogenicity(
    ...
    """
    Analyze correlations between PPL and immunogenicity.

    Raw PPL values are used directly:
    - Lower PPL = more human-like = lower immunogenicity = better
    - Expect negative correlations: Higher PPL → Higher immunogenicity

    Args:
        df: DataFrame with PPL values and immunogenicity column
        target_column: Name of immunogenicity column
    ...
    # Get target values (immunogenicity)
    ...
    for model in model_columns:
        ...
        # Use raw PPL values (no direction correction)
        x = df[model].values
...
def print_results_table(
    ...
    print(f"\n{'='*90}")
    print("IMMUNOGENICITY CORRELATION RESULTS")
    print(f"{'='*90}")
...
def main():
    parser = argparse.ArgumentParser(
        description='Create immunogenicity correlation plots with bootstrapping',
...
Correlation Direction:
    Raw PPL values are used directly:
    - Lower PPL = more human-like sequence = better
    - Lower immunogenicity = less immune response = better
    - Expect negative correlations (Higher PPL → Higher immunogenicity)
        """
    )
    ...
    parser.add_argument('--target_column', type=str, default=None,
                        help='Name of immunogenicity column (auto-detect if not specified)')
    parser.add_argument('--n_bootstrap', type=int, default=1000,
                        help='Number of bootstrap iterations (default: 1000)')
    parser.add_argument('--dpi', type=int, default=300,
                        help='DPI for saved figure (default: 300)')
    parser.add_argument('--output_filename', type=str, default='developability_immunogenicity.png',
                        help='Output filename (default: developability_immunogenicity.png)')
    ...
    # Load data
    print("=" * 80)
    print("Immunogenicity Correlation Analysis with Bootstrapping")
    print("=" * 80)
    ...
    if args.ablation_mode:
        ...
        # PPL column mapping for ablation models
        model_columns = []
        display_names = []
        colors = []

        # Best model (PRISM Full) - use marginalized PPL for immunogenicity
        prism_ppl_col = 'evo_ab_ppl_marginalized'
        if prism_ppl_col in df.columns:
            ...
            print(f"  ✓ PRISM Full PPL loaded from: {args.csv_path} (column: {prism_ppl_col})")
        elif 'evo_ab_ppl' in df.columns:
            # Fallback to evo_ab_ppl if marginalized not available
            df['prism_full_ppl'] = df['evo_ab_ppl']
            model_columns.append('prism_full_ppl')
            display_names.append('PRISM Full')
            colors.append(ABLATION_COLORS['PRISM Full'])
            print(f"  ✓ PRISM Full PPL loaded from: {args.csv_path} (column: evo_ab_ppl - fallback)")
        else:
            print(f"  ✗ WARNING: No PRISM PPL column found in {args.csv_path}")
        ...
        for csv_path, col_name, display_name in ablation_files:
            if csv_path and os.path.exists(csv_path):
                ...
                if ppl_col is not None:
                    ...
                    print(f"  ✓ {display_name} PPL loaded from: {csv_path} (column: {ppl_col_name})")
                else:
                    print(f"  ✗ WARNING: No valid PPL column found in {csv_path}")
            elif csv_path:
        ...
        if args.target_column:
            ...
            possible_targets = ['immunogenicity', 'immunogenicity_score', 'score', 'value']
            target_column = None
            ...
            if target_column is None:
                ...
                non_ppl_cols = [c for c in numeric_cols if '_ppl' not in c.lower()]
                if non_ppl_cols:
                    target_column = non_ppl_cols[0]
        ...
        for model, display in zip(model_columns, display_names):
            corr, ci_low, ci_high, p = spearman_correlations[model]
            vs_best = format_pvalue(spearman_pvalues[model]) if model != 'prism_full_ppl' else "-"
            print(f"{display:<25} {corr:>12.4f} [{ci_low:>8.4f}, {ci_high:>8.4f}] {format_pvalue(p):>15} {vs_best:>15}")

        # Create figure
        if args.pearson_only:
            # Single Pearson plot
            fig, ax = plt.subplots(1, 1, figsize=(8, 8))
            fig.suptitle('Immunogenicity (Ablation Study)', fontsize=FONT_CONFIG['title_size'],
                        fontweight=FONT_CONFIG['title_weight'], y=0.98)
            ...
            output_filename = 'immunogenicity_ablation_pearson.png'
        else:
            ...
            output_filename = 'immunogenicity_ablation.png'
    ...
    existing_display = [display_names[model_columns.index(m)] for m in existing_models]
    existing_colors = [colors[model_columns.index(m)] for m in existing_models]
    ...
    if args.target_column:
        ...
        # Try common immunogenicity column names
        possible_targets = ['immunogenicity', 'immunogenicity_score', 'score', 'value',
        ...
        # If still not found, use the first non-PPL numeric column
        if target_column is None:
            numeric_cols = df.select_dtypes(include=[np.number]).columns.tolist()
            non_ppl_cols = [c for c in numeric_cols if '_ppl' not in c.lower()]
            if non_ppl_cols:
        ...
        if target_column is None:
            print("\nERROR: Could not auto-detect target column!")
            print("Please specify --target_column")
            return

    print(f"\n  Target column: {target_column}")
    print(f"  Using raw PPL values (no direction correction)")
    print(f"  (Expect negative correlations: Higher PPL → Higher immunogenicity)")
    ...
    # Count how many PRISM variants actually exist in the data
    n_prism_variants_existing = sum(1 for m in existing_models if m in model_columns[:n_prism_variants])

    spearman_corrs, pearson_corrs, spearman_pvals, pearson_pvals = analyze_immunogenicity(
        df=df,
        target_column=target_column,
        model_columns=existing_models,
        n_bootstrap=args.n_bootstrap,
        n_prism_variants=n_prism_variants_existing
    )
    ...
    if args.pearson_only:
        ...
        fig.suptitle('Immunogenicity', fontsize=FONT_CONFIG['title_size'],
                     fontweight=FONT_CONFIG['title_weight'], y=0.98)
        create_correlation_barplot(
            pearson_corrs,
            pearson_pvals,
        ...
        output_filename = args.output_filename.replace('.png', '_pearson.png')
    else:
        # Two subplots (Spearman + Pearson)
        if args.vertical_layout:
            fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(10, 14))
